...
parser.add_argument('--feature-of-interest', type=str, default='evals/charts/episode_reward',
...
parser.add_argument('--y-label', type=str, default="Episode Return",
...
# }
exp_convert_dict = {
    'On-PIAM-MR': 'On-PIAM-MR',
    'Off-PIAM-MR': 'Off-PIAM-MR',
    'CO-IAM-MR': 'CO-IAM-MR',
}
...
if not path.exists(f"{feature_name}/all_df_cache.pkl"):
    ...
    for idx, run in enumerate(runs):
        if args.feature_of_interest in run.summary:
            ...
            # hacks
            if exp_name == "On-PIAM" or exp_name == "Off-PIAM" or exp_name == "CO-IAM":
                metrics_dataframe = run.history(keys=[args.feature_of_interest, 'global_step'], samples=args.samples)
                exp_name = exp_name + "-MR"
                ...
                metrics_dataframe[args.feature_of_interest] = metrics_dataframe[args.feature_of_interest]
...
def export_legend(ax, filename="legend.pdf"):
    ...
    legend = ax2.legend(handles=handles, labels=labels, frameon=False, loc='lower center', ncol=3, fontsize=20, handlelength=1)
