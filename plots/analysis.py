...
runs = api.runs("drhou/invalid-action-masking")
analysis = True
summary_list = [] 
config_list = [] 
name_list = [] 
for run in runs: 
    ...
    if analysis:
        ...
        history['rollling_e'] = history['charts/episode_reward'].dropna().rolling(1).mean()
        first_best_reward_idx = (history["rollling_e"] >= 40.0).idxmax()
        if history.iloc[first_best_reward_idx]["rollling_e"] >= 40.0:
            summary_json_dict["first_learned_timestep"] = history.iloc[first_best_reward_idx]["global_step"] / 1000000
        else:
            summary_json_dict["first_learned_timestep"] = 0
        summary_json_dict["first_reward_timestep"] = history.iloc[(history['charts/episode_reward'] > 0).idxmax()]["global_step"] / 1000000

        if run.config["exp_name"] == "Penalty":
            run.config["exp_name"] += "-" + str(run.config['invalid_action_penalty'])

        # mask removed logic
        if run.config["exp_name"] == "On-PIAM" or run.config["exp_name"] == "Off-PIAM" or run.config["exp_name"] == "CO-IAM":
            history['evals_rollling_e'] = history['evals/charts/episode_reward'].dropna().rolling(1).mean()
            first_best_reward_idx = (history["evals_rollling_e"] >= 40.0).idxmax()
            if history.iloc[first_best_reward_idx]["evals_rollling_e"] >= 40.0:
                summary_json_dict["evals_first_learned_timestep"] = history.iloc[first_best_reward_idx]["global_step"] / 1000000
            else:
                summary_json_dict["evals_first_learned_timestep"] = 0
            summary_json_dict["evals_first_reward_timestep"] = history.iloc[(history['charts/episode_reward'] > 0).idxmax()]["global_step"] / 1000000
...
# mask removal
mask_removed = all_df[all_df["exp_name"]=="On-PIAM"].copy()
...
mask_removed["exp_name"] = "On-PIAM-MR"
all_df = all_df.append(mask_removed, ignore_index=True)

mask_removed = all_df[all_df["exp_name"]=="Off-PIAM"].copy()
mask_removed['charts/episode_reward'] = mask_removed['evals/charts/episode_reward']
mask_removed['stats/num_invalid_action_null'] = mask_removed['evals/stats/num_invalid_action_null']
mask_removed['stats/num_invalid_action_busy_unit'] = mask_removed['evals/stats/num_invalid_action_busy_unit']
mask_removed['stats/num_invalid_action_ownership'] = mask_removed['evals/stats/num_invalid_action_ownership']
mask_removed['first_learned_timestep'] = mask_removed['evals_first_learned_timestep']
mask_removed['first_reward_timestep'] = mask_removed['evals_first_reward_timestep']
mask_removed["exp_name"] = "Off-PIAM-MR"
all_df = all_df.append(mask_removed, ignore_index=True)

mask_removed = all_df[all_df["exp_name"]=="CO-IAM"].copy()
mask_removed['charts/episode_reward'] = mask_removed['evals/charts/episode_reward']
mask_removed['stats/num_invalid_action_null'] = mask_removed['evals/stats/num_invalid_action_null']
mask_removed['stats/num_invalid_action_busy_unit'] = mask_removed['evals/stats/num_invalid_action_busy_unit']
mask_removed['stats/num_invalid_action_ownership'] = mask_removed['evals/stats/num_invalid_action_ownership']
mask_removed['first_learned_timestep'] = mask_removed['evals_first_learned_timestep']
mask_removed['first_reward_timestep'] = mask_removed['evals_first_reward_timestep']
mask_removed["exp_name"] = "CO-IAM-MR"
