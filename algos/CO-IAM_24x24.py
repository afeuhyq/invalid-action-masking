...
# taken from https://github.com/vwxyzjn/invalid-action-masking
...
if __name__ == "__main__":
    ...
    parser.add_argument('--device', type=str, default="cpu",
                       help='cpu, cuda:0 or cuda:1')  # drhou
...
device = torch.device(args.device)  # drhou
# device = torch.device('cuda' if torch.cuda.is_available() and args.cuda else 'cpu')  # drhou
...
class CategoricalMasked(Categorical):
    def __init__(self, probs=None, logits=None, validate_args=None, masks=[]):
        ...
        if len(self.masks) == 0:
            ...
            categorical = Categorical(logits=logits)  # MODIFIED: Action-Level Masking
            unmasked_probs = categorical.probs
            self.masks = masks.type(torch.BoolTensor).to(device)
            mask_replace_zero = torch.where(self.masks, masks.to(device), torch.tensor(1e-38).to(device))
            masked_probs = torch.mul(unmasked_probs, mask_replace_zero)
            super(CategoricalMasked, self).__init__(masked_probs, None, validate_args)
